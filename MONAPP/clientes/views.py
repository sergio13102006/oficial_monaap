...
from .archivo_clientes import build_clientes_excel_response, build_clientes_pdf_response


def _obtener_clientes_filtrados(request):
...
def lista_clientes(request):
    ...
    """
    filtros = _obtener_clientes_filtrados(request)
...
@login_required
def imprimir_clientes(request):
    filtros = _obtener_clientes_filtrados(request)
    context = {
        **filtros,
        'fecha_impresion': date.today(),
    }
    return render(request, 'clientes/imprimir_clientes.html', context)


@login_required
def crear_archivo_clientes(request):
    filtros = _obtener_clientes_filtrados(request)
    clientes = list(filtros["clientes"])
    formato = (request.GET.get("formato") or "pdf").strip().lower()

    if formato == "excel":
        return build_clientes_excel_response(clientes)

    return build_clientes_pdf_response(clientes)
