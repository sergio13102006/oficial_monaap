...
def _nombre_archivo_proveedores(extension):
    fecha = datetime.now().strftime("%Y%m%d")
    return f"archivo_proveedores_monakeratina_{slugify(fecha)}.{extension}"
...
def build_proveedores_pdf_response(proveedores):
    buffer = BytesIO()
    ...
    header_table = Table(
        [[logo_flowable, Paragraph("Archivo de proveedores", title_style), ""]],
    ...
    top_info = Table(
        [[
            Table([[Paragraph("NUMERO", small_label_style)], [Paragraph(f"{len(proveedores):04d}", small_value_style)]], colWidths=[28 * mm]),
            Table([
                [Paragraph("DE", small_label_style)],
                [Paragraph("Monakeratina", small_value_style)],
                [Paragraph("Sistema de proveedores", styles["BodyText"])],
                [Paragraph("Archivo interno", styles["BodyText"])],
            ], colWidths=[64 * mm]),
            Table([
                [Paragraph("PARA", small_label_style)],
                [Paragraph("Proveedores registrados", small_value_style)],
                [Paragraph("Base de proveedores de MONAAPP", styles["BodyText"])],
            ], colWidths=[74 * mm]),
        ]],
    ...
    meta_table = Table(
        [[
            ...
            Table([[Paragraph("MODULO", small_label_style)], [Paragraph("Proveedores", small_value_style)]], colWidths=[28 * mm]),
    ...
    table_data = [["NIT", "Proveedor", "Telefono", "Correo", "Estado"]]
    if proveedores:
        for proveedor in proveedores:
            table_data.append([
                str(proveedor.nit or "-"),
                str(proveedor.nombre_proveedor or "-"),
                str(proveedor.telefono_proveedor or "-"),
                str(proveedor.correo_proveedor or "-"),
                str((proveedor.estado or "").capitalize() or "-"),
            ])
    else:
        table_data.append(["No hay proveedores registrados.", "", "", "", ""])

    detail_table = Table(
        table_data,
        colWidths=[28 * mm, 54 * mm, 30 * mm, 46 * mm, 18 * mm],
        repeatRows=1,
    )
    ...
    total_table = Table(
        [["Total registros", str(len(proveedores))]],
    ...
    response["Content-Disposition"] = f'attachment; filename="{_nombre_archivo_proveedores("pdf")}"'
    return response


def build_proveedores_excel_response(proveedores):
    wb = Workbook()
    ws = wb.active
    ...
    info_rows = [
        ...
        (6, "Modulo", "Proveedores"),
        (7, "Estado", "Generado"),
        (8, "Total de proveedores", len(proveedores)),
    ]
    ...
    style_range(11, 11, 2, 6, fill=dark_fill, border=thin_border, alignment=left)
    ws.merge_cells("B11:F11")
    ws["B11"] = "DETALLE DE PROVEEDORES"
    apply_style(ws["B11"], font=section_font, alignment=left)

    header_row = 12
    headers = ["NIT", "Proveedor", "Telefono", "Correo", "Estado"]
    ...
    if proveedores:
        for proveedor in proveedores:
            values = [
                proveedor.nit or "-",
                proveedor.nombre_proveedor or "-",
                proveedor.telefono_proveedor or "-",
                proveedor.correo_proveedor or "-",
                (proveedor.estado or "").capitalize() or "-",
            ]
        ...
        ws.cell(row=current_row, column=2, value="No hay proveedores registrados.")
        style_range(current_row, current_row, 2, 6, fill=soft_fill_2, border=thin_border, alignment=center)
    ...
    apply_style(
        ws.cell(row=total_row, column=6, value=len(proveedores)),
        font=label_font,
        fill=soft_fill,
        border=thin_border,
        alignment=right,
    ...
    style_range(footer_row, footer_row, 2, 6, alignment=center)
    ...
    ws.print_area = f"B1:F{footer_row}"

    response = HttpResponse(
        content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    response["Content-Disposition"] = f'attachment; filename="{_nombre_archivo_proveedores("xlsx")}"'
    wb.save(response)
    return response
