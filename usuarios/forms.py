...
class IdentifierPasswordResetForm(PasswordResetForm):
    ...
    def get_users(self, email):
        ...
        for user in User._default_manager.filter(
            ...
            if user.has_usable_password() and (user.email or '').strip():
                yield user
    ...
    def clean_email(self):
        ...
        if not usuarios:
            raise forms.ValidationError(
                'No encontramos una cuenta recuperable con ese correo, usuario o documento.'
            )
...
class LoginForm(AuthenticationForm):
    username = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': 'Documento, usuario o correo',
            'autofocus': True
        })
    )
    ...
    def _resolver_identificador(self, valor):
        valor = (valor or '').strip()
        if not valor:
            return valor

        usuario = User._default_manager.filter(
            Q(username__iexact=valor)
            | Q(email__iexact=valor)
            | Q(perfil__documento__iexact=valor),
            is_active=True,
        ).distinct().first()
        return usuario.username if usuario else valor

    def _buscar_candidatos(self, valor):
        valor = (valor or '').strip()
        if not valor:
    ...
    def clean(self):
        ...
        if username and password:
            username_resuelto = self._resolver_identificador(username)
            self.user_cache = authenticate(
                self.request,
                username=username_resuelto,
                password=password,
            )

            if self.user_cache is None:
                candidatos = list(self._buscar_candidatos(username))
                if not candidatos:
                    raise forms.ValidationError(
                        'No existe una cuenta activa con ese usuario, correo o documento.',
                        code='invalid_login',
                    )
            ...
            if self.user_cache is None:
                if not self._buscar_candidatos(username):
                    raise forms.ValidationError(
                        'No existe una cuenta activa con ese usuario, correo o documento.',
                        code='invalid_login',
                    )
                raise forms.ValidationError(
                    'La contraseña no coincide con la cuenta encontrada.',
                    code='invalid_login',
                )
